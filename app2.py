...
def predict_diabetes_risk(patient_data, model, preprocessor, threshold):
    ...
    try:
        ...
        # Check preprocessor type and methods
        st.write(f"🔧 Preprocessor type: {type(preprocessor)}")
        st.write(f"🔧 Has preprocess_new_data: {hasattr(preprocessor, 'preprocess_new_data')}")
        ...
        if not hasattr(preprocessor, 'preprocess_new_data'):
            ...
            if hasattr(preprocessor, 'transform'):
                st.write("🔄 Using transform method instead...")
                # Convert to DataFrame if needed
                import pandas as pd
                df = pd.DataFrame([patient_data])
                patient_processed = preprocessor.transform(df)
                uncertainty_info = {'overall_confidence': [1.0], 'imputed_features': []}
            else:
                raise AttributeError("Preprocessor doesn't have required methods")
        else:
            st.write("🔄 Calling preprocess_new_data...")
            ...
            try:
                ...
                st.write("✅ preprocess_new_data with return_uncertainty=True succeeded")
            except Exception as e1:
                st.write(f"❌ preprocess_new_data with return_uncertainty=True failed: {e1}")
                try:
                    # Try without return_uncertainty
                    patient_processed = preprocessor.preprocess_new_data(patient_data)
                    uncertainty_info = {'overall_confidence': [1.0], 'imputed_features': []}
                    st.write("✅ preprocess_new_data without return_uncertainty succeeded")
                except Exception as e2:
                    st.write(f"❌ preprocess_new_data without return_uncertainty failed: {e2}")
                    raise e2
        
        st.write("🔄 Data preprocessing completed")
        st.write(f"📊 Processed data shape: {patient_processed.shape}")
        
        # Get probability
        st.write("🔄 Getting prediction probability...")
        risk_prob = model.predict_proba(patient_processed)[0, 1]
        st.write(f"📊 Risk probability: {risk_prob}")
        
        # Apply optimal threshold
        prediction = int(risk_prob >= threshold)
        st.write(f"📊 Prediction (threshold {threshold}): {prediction}")
        ...
        st.write(f"❌ Error in predict_diabetes_risk: {str(e)}")
        import traceback
